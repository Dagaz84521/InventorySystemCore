...
{
	/** Resolve only within the supplied inventory identity. Returned pointers are short-lived. */
	INVENTORYCORE_API const FInventoryEntry* Resolve(const TArray<FInventoryEntry>& Entries,
		const FGuid& InventoryID, const FInventoryHandle& Handle);
	INVENTORYCORE_API FInventoryEntry* Resolve(TArray<FInventoryEntry>& Entries,
		const FGuid& InventoryID, const FInventoryHandle& Handle);
