...
└── Source/InventoryCore
    ...
    │       ├── InventoryEntryArray.h        # EntryArray + EntryHandle
    │       ├── InventoryEntry.h
...
外部使用 `FInventoryEntryHandle`：组件入口先检查 `ParentInventory`，再进入 ID 操作。操作结果和事件使用同一种 Handle。
...
- `FInventoryEntryHandle` 是唯一的 Entry Handle，由组件引用和 EntryID 组成。
- `IsSet` 只检查字段是否完整；`IsValid` 会向所属组件实际解析记录。
- Handle 标识 Entry，不追踪该 Entry 当前存放的物品。换物品或清空记录不改变身份；删除记录后解析失败。
- 组件复制后，原组件的 Handle 不会被副本接受；重新注册同一个组件不改变 Handle 的归属。
- ID 分配高水位不能在同一库存生命周期内回退。直接使用底层数组算法时，调用方必须管理 `NextID`，且不能让已删除记录的 ID 被重新使用，否则旧 Handle 可能再次指向新记录。
- 当前 Handle 不承诺跨存档或网络同步稳定性。Handle 中的组件引用具有原有的 UObject 引用语义；查询返回的 Entry 指针不能跨数组结构变化保存。
